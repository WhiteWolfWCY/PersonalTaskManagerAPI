...
export const createValidator: ValidationChain[] = [
    ...
    body('description')
        .trim()
        .isString()
        .withMessage('The task description needs to be in a text format'),
    body('priority')
        .trim()
        .isIn([Priority.normal, Priority.high, Priority.low])
        .withMessage('The task priority can only be normal, high or low'),
    body('status')
        .trim()
        .isIn([Status.todo, Status.inProgress, Status.completed])
        .withMessage('The task status can only be to do, in progress or completed'),
];
